...
- Selections in apps whose accessibility tree reports text shifted or without spaces can now be read through that app's own Copy command. Turn on **Clipboard-assisted reading** in Settings > **Selection Assistant**, then list the apps it applies to; it is off by default, and your clipboard is backed up and put back right after each read.
