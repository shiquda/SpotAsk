...
struct SpotAskIntentTests {
    ...
    @Test @MainActor func coldStartAddToChatAppendsNewlinesAndPlacesCaretAtEnd() async {
        ...
        #expect(panel.composerTextView?.selectedRange() == NSRange(location: 7, length: 0))
    }
    ...
    @Test @MainActor func addToChatQuotesEachLineAndPreservesBlankLines() async {
        let commandCenter = SpotAskCommandCenter()
        let panel = HostingPanelController()
        let settings = makeSettings()
        let viewModel = ChatViewModel(
        ...
        let expected = "> first\n>\n> third\n\n"
        #expect(panel.composerTextView?.string == expected)
        #expect(panel.composerTextView?.selectedRange() == NSRange(location: (expected as NSString).length, length: 0))
    }

    @Test @MainActor func addToChatAppendsQuotedTextAfterExistingDraftAndClearsPreset() async {
        let commandCenter = SpotAskCommandCenter()
        let panel = HostingPanelController()
        let settings = makeSettings()
        let viewModel = ChatViewModel(
