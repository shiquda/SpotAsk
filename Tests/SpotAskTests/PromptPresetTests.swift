...
struct PromptPresetTests {
    ...
    @Test("Adjacent prompt moves update order, stop at boundaries, and persist")
    func adjacentPromptMovesPersistAndRespectBoundaries() {
        let suiteName = "PromptPresetTests.\(UUID().uuidString)"
        let defaults = UserDefaults(suiteName: suiteName)!
        defer { defaults.removePersistentDomain(forName: suiteName) }
        let settings = AppSettings(defaults: defaults)
        let initialIDs = settings.promptPresets.map(\.id)
        let firstID = try! #require(initialIDs.first)
        let secondID = try! #require(initialIDs.dropFirst().first)
        let thirdID = try! #require(initialIDs.dropFirst(2).first)
        let finalID = try! #require(initialIDs.last)

        #expect(settings.movePromptPreset(id: secondID, by: -1))
        #expect(settings.promptPresets.prefix(2).map(\.id) == [secondID, firstID])
        #expect(AppSettings(defaults: defaults).promptPresets.map(\.id) == settings.promptPresets.map(\.id))

        #expect(settings.movePromptPreset(id: firstID, by: 1))
        #expect(settings.promptPresets.prefix(3).map(\.id) == [secondID, thirdID, firstID])
        #expect(AppSettings(defaults: defaults).promptPresets.map(\.id) == settings.promptPresets.map(\.id))

        let orderBeforeBoundaryMoves = settings.promptPresets.map(\.id)
        #expect(!settings.movePromptPreset(id: secondID, by: -1))
        #expect(!settings.movePromptPreset(id: finalID, by: 1))
        #expect(settings.promptPresets.map(\.id) == orderBeforeBoundaryMoves)
    }
