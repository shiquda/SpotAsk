...
部分应用（例如 Zotero 的 PDF 阅读器）向 macOS 辅助功能汇报选中文字时，可能出现缺失空格或边界错位的问题。遇到这类应用时，可以在设置 > **划词助手**中开启“剪贴板辅助划词”，并选择适用的应用。

开启后，划词检测与操作栏的弹出方式保持不变，操作栏出现时不会动剪贴板。只有在你使用某个动作时，SpotAsk 才会像应用的复制命令一样短暂复制选区，读取其中的准确文字，并立即恢复原有的剪贴板内容。

该功能默认关闭。未选中文本时绝不会触发复制，未在列表中的应用依然保持正常的辅助功能读取。
